...
/**
 * Tests OpenAPI compliance for Commands endpoints.
 *
 * This test class dynamically validates that all Commands-tagged endpoints
 * in the OpenAPI spec have corresponding SDK methods and tests.
 */
class CommandsTest extends OpenAPITestCase
{
    protected static string $category = 'Commands';

    public function test_all_commands_endpoints_have_sdk_methods()
    {
        ...
        if (! empty($uncovered)) {
            $this->fail(
                "The following ".count($uncovered)." Commands endpoints do not have SDK methods:\n".
                implode("\n", $uncovered)
            );
        }
    ...
    public function test_all_commands_sdk_methods_have_tests()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        ...
        if (! empty($missingTests)) {
            $this->fail(
                "The following ".count($missingTests)." Commands SDK methods do not have tests:\n".
                implode("\n", $missingTests)
            );
        }
    ...
    public function test_commands_endpoint_coverage_statistics()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        $testedMethods = static::getTestedMethods();
        ...
        echo str_repeat('=', strlen("Commands API Coverage") + 8) . "\n\n";
    }
}
