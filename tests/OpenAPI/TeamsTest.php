...
/**
 * Tests OpenAPI compliance for Teams endpoints.
 *
 * This test class dynamically validates that all Teams-tagged endpoints
 * in the OpenAPI spec have corresponding SDK methods and tests.
 */
class TeamsTest extends OpenAPITestCase
{
    protected static string $category = 'Teams';

    public function test_all_teams_endpoints_have_sdk_methods()
    {
        ...
        if (! empty($uncovered)) {
            $this->fail(
                "The following ".count($uncovered)." Teams endpoints do not have SDK methods:\n".
                implode("\n", $uncovered)
            );
        }
    ...
    public function test_all_teams_sdk_methods_have_tests()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        ...
        if (! empty($missingTests)) {
            $this->fail(
                "The following ".count($missingTests)." Teams SDK methods do not have tests:\n".
                implode("\n", $missingTests)
            );
        }
    ...
    public function test_teams_endpoint_coverage_statistics()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        $testedMethods = static::getTestedMethods();
        ...
        echo str_repeat('=', strlen("Teams API Coverage") + 8) . "\n\n";
    }
}
