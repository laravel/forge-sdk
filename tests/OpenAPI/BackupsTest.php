...
/**
 * Tests OpenAPI compliance for Backups endpoints.
 *
 * This test class dynamically validates that all Backups-tagged endpoints
 * in the OpenAPI spec have corresponding SDK methods and tests.
 */
class BackupsTest extends OpenAPITestCase
{
    protected static string $category = 'Backups';

    public function test_all_backups_endpoints_have_sdk_methods()
    {
        ...
        if (! empty($uncovered)) {
            $this->fail(
                'The following '.count($uncovered)." Backups endpoints do not have SDK methods:\n".
                implode("\n", $uncovered)
            );
        }
    ...
    public function test_all_backups_sdk_methods_have_tests()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        ...
        if (! empty($missingTests)) {
            $this->fail(
                'The following '.count($missingTests)." Backups SDK methods do not have tests:\n".
                implode("\n", $missingTests)
            );
        }
    ...
    public function test_backups_endpoint_coverage_statistics()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        $testedMethods = static::getTestedMethods();
        ...
        echo str_repeat('=', strlen('Backups API Coverage') + 8)."\n\n";
    }
}
