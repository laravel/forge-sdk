...
/**
 * Tests OpenAPI compliance for Monitors endpoints.
 *
 * This test class dynamically validates that all Monitors-tagged endpoints
 * in the OpenAPI spec have corresponding SDK methods and tests.
 */
class MonitorsTest extends OpenAPITestCase
{
    protected static string $category = 'Monitors';

    public function test_all_monitors_endpoints_have_sdk_methods()
    {
        ...
        if (! empty($uncovered)) {
            $this->fail(
                'The following '.count($uncovered)." Monitors endpoints do not have SDK methods:\n".
                implode("\n", $uncovered)
            );
        }
    ...
    public function test_all_monitors_sdk_methods_have_tests()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        ...
        if (! empty($missingTests)) {
            $this->fail(
                'The following '.count($missingTests)." Monitors SDK methods do not have tests:\n".
                implode("\n", $missingTests)
            );
        }
    ...
    public function test_monitors_endpoint_coverage_statistics()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        $testedMethods = static::getTestedMethods();
        ...
        echo str_repeat('=', strlen('Monitors API Coverage') + 8)."\n\n";
    }
}
