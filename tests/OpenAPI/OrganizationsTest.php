...
/**
 * Tests OpenAPI compliance for Organizations endpoints.
 *
 * This test class dynamically validates that all Organizations-tagged endpoints
 * in the OpenAPI spec have corresponding SDK methods and tests.
 */
class OrganizationsTest extends OpenAPITestCase
{
    protected static string $category = 'Organizations';

    public function test_all_organizations_endpoints_have_sdk_methods()
    {
        ...
        if (! empty($uncovered)) {
            $this->fail(
                "The following ".count($uncovered)." Organizations endpoints do not have SDK methods:\n".
                implode("\n", $uncovered)
            );
        }
    ...
    public function test_all_organizations_sdk_methods_have_tests()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        ...
        if (! empty($missingTests)) {
            $this->fail(
                "The following ".count($missingTests)." Organizations SDK methods do not have tests:\n".
                implode("\n", $missingTests)
            );
        }
    ...
    public function test_organizations_endpoint_coverage_statistics()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        $testedMethods = static::getTestedMethods();
        ...
        echo "\n\n";
        echo "=== Organizations API Coverage ===\n";
        ...
        echo str_repeat('=', strlen("Organizations API Coverage") + 8) . "\n\n";
    }
}
