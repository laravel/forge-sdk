...
/**
 * Tests OpenAPI compliance for Redirect Rules endpoints.
 *
 * This test class dynamically validates that all Redirect Rules-tagged endpoints
 * in the OpenAPI spec have corresponding SDK methods and tests.
 */
class RedirectRulesTest extends OpenAPITestCase
{
    protected static string $category = 'Redirect Rules';

    public function test_all_redirect_rules_endpoints_have_sdk_methods()
    {
        ...
        if (! empty($uncovered)) {
            $this->fail(
                'The following '.count($uncovered)." Redirect Rules endpoints do not have SDK methods:\n".
                implode("\n", $uncovered)
            );
        }
    ...
    public function test_all_redirect_rules_sdk_methods_have_tests()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        ...
        if (! empty($missingTests)) {
            $this->fail(
                'The following '.count($missingTests)." Redirect Rules SDK methods do not have tests:\n".
                implode("\n", $missingTests)
            );
        }
    ...
    public function test_redirect_rules_endpoint_coverage_statistics()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        $testedMethods = static::getTestedMethods();
        ...
        echo "\n\n";
        echo "=== Redirect Rules API Coverage ===\n";
        ...
        echo str_repeat('=', strlen('Redirect Rules API Coverage') + 8)."\n\n";
    }
}
