...
/**
 * Tests OpenAPI compliance for Servers endpoints.
 *
 * This test class dynamically validates that all Servers-tagged endpoints
 * in the OpenAPI spec have corresponding SDK methods and tests.
 */
class ServersTest extends OpenAPITestCase
{
    protected static string $category = 'Servers';

    public function test_all_servers_endpoints_have_sdk_methods()
    {
        ...
        if (! empty($uncovered)) {
            $this->fail(
                "The following ".count($uncovered)." Servers endpoints do not have SDK methods:\n".
                implode("\n", $uncovered)
            );
        }
    ...
    public function test_all_servers_sdk_methods_have_tests()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        ...
        if (! empty($missingTests)) {
            $this->fail(
                "The following ".count($missingTests)." Servers SDK methods do not have tests:\n".
                implode("\n", $missingTests)
            );
        }
    ...
    public function test_servers_endpoint_coverage_statistics()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        $testedMethods = static::getTestedMethods();
        ...
        echo str_repeat('=', strlen("Servers API Coverage") + 8) . "\n\n";
    }
}
