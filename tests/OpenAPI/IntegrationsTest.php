...
/**
 * Tests OpenAPI compliance for Integrations endpoints.
 *
 * This test class dynamically validates that all Integrations-tagged endpoints
 * in the OpenAPI spec have corresponding SDK methods and tests.
 */
class IntegrationsTest extends OpenAPITestCase
{
    protected static string $category = 'Integrations';

    public function test_all_integrations_endpoints_have_sdk_methods()
    {
        ...
        if (! empty($uncovered)) {
            $this->fail(
                "The following ".count($uncovered)." Integrations endpoints do not have SDK methods:\n".
                implode("\n", $uncovered)
            );
        }
    ...
    public function test_all_integrations_sdk_methods_have_tests()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        ...
        if (! empty($missingTests)) {
            $this->fail(
                "The following ".count($missingTests)." Integrations SDK methods do not have tests:\n".
                implode("\n", $missingTests)
            );
        }
    ...
    public function test_integrations_endpoint_coverage_statistics()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        $testedMethods = static::getTestedMethods();
        ...
        echo "\n\n";
        echo "=== Integrations API Coverage ===\n";
        ...
        echo str_repeat('=', strlen("Integrations API Coverage") + 8) . "\n\n";
    }
}
