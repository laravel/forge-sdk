...
/**
 * Tests OpenAPI compliance for SSH Keys endpoints.
 *
 * This test class dynamically validates that all SSH Keys-tagged endpoints
 * in the OpenAPI spec have corresponding SDK methods and tests.
 */
class SSHKeysTest extends OpenAPITestCase
{
    protected static string $category = 'SSH Keys';

    public function test_all_ssh_keys_endpoints_have_sdk_methods()
    {
        ...
        if (! empty($uncovered)) {
            $this->fail(
                "The following ".count($uncovered)." SSH Keys endpoints do not have SDK methods:\n".
                implode("\n", $uncovered)
            );
        }
    ...
    public function test_all_ssh_keys_sdk_methods_have_tests()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        ...
        if (! empty($missingTests)) {
            $this->fail(
                "The following ".count($missingTests)." SSH Keys SDK methods do not have tests:\n".
                implode("\n", $missingTests)
            );
        }
    ...
    public function test_ssh_keys_endpoint_coverage_statistics()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        $testedMethods = static::getTestedMethods();
        ...
        echo str_repeat('=', strlen("SSH Keys API Coverage") + 8) . "\n\n";
    }
}
