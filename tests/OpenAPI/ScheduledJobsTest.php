...
/**
 * Tests OpenAPI compliance for Scheduled Jobs endpoints.
 *
 * This test class dynamically validates that all Scheduled Jobs-tagged endpoints
 * in the OpenAPI spec have corresponding SDK methods and tests.
 */
class ScheduledJobsTest extends OpenAPITestCase
{
    protected static string $category = 'Scheduled Jobs';

    public function test_all_scheduled_jobs_endpoints_have_sdk_methods()
    {
        ...
        if (! empty($uncovered)) {
            $this->fail(
                "The following ".count($uncovered)." Scheduled Jobs endpoints do not have SDK methods:\n".
                implode("\n", $uncovered)
            );
        }
    ...
    public function test_all_scheduled_jobs_sdk_methods_have_tests()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        ...
        if (! empty($missingTests)) {
            $this->fail(
                "The following ".count($missingTests)." Scheduled Jobs SDK methods do not have tests:\n".
                implode("\n", $missingTests)
            );
        }
    ...
    public function test_scheduled_jobs_endpoint_coverage_statistics()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        $testedMethods = static::getTestedMethods();
        ...
        echo "\n\n";
        echo "=== Scheduled Jobs API Coverage ===\n";
        ...
        echo str_repeat('=', strlen("Scheduled Jobs API Coverage") + 8) . "\n\n";
    }
}
