...
/**
 * Tests OpenAPI compliance for Recipes endpoints.
 *
 * This test class dynamically validates that all Recipes-tagged endpoints
 * in the OpenAPI spec have corresponding SDK methods and tests.
 */
class RecipesTest extends OpenAPITestCase
{
    protected static string $category = 'Recipes';

    public function test_all_recipes_endpoints_have_sdk_methods()
    {
        ...
        if (! empty($uncovered)) {
            $this->fail(
                'The following '.count($uncovered)." Recipes endpoints do not have SDK methods:\n".
                implode("\n", $uncovered)
            );
        }
    ...
    public function test_all_recipes_sdk_methods_have_tests()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        ...
        if (! empty($missingTests)) {
            $this->fail(
                'The following '.count($missingTests)." Recipes SDK methods do not have tests:\n".
                implode("\n", $missingTests)
            );
        }
    ...
    public function test_recipes_endpoint_coverage_statistics()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        $testedMethods = static::getTestedMethods();
        ...
        echo str_repeat('=', strlen('Recipes API Coverage') + 8)."\n\n";
    }
}
