...
/**
 * Tests OpenAPI compliance for Firewall Rules endpoints.
 *
 * This test class dynamically validates that all Firewall Rules-tagged endpoints
 * in the OpenAPI spec have corresponding SDK methods and tests.
 */
class FirewallRulesTest extends OpenAPITestCase
{
    protected static string $category = 'Firewall Rules';

    public function test_all_firewall_rules_endpoints_have_sdk_methods()
    {
        ...
        if (! empty($uncovered)) {
            $this->fail(
                'The following '.count($uncovered)." Firewall Rules endpoints do not have SDK methods:\n".
                implode("\n", $uncovered)
            );
        }
    ...
    public function test_all_firewall_rules_sdk_methods_have_tests()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        ...
        if (! empty($missingTests)) {
            $this->fail(
                'The following '.count($missingTests)." Firewall Rules SDK methods do not have tests:\n".
                implode("\n", $missingTests)
            );
        }
    ...
    public function test_firewall_rules_endpoint_coverage_statistics()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        $testedMethods = static::getTestedMethods();
        ...
        echo "\n\n";
        echo "=== Firewall Rules API Coverage ===\n";
        ...
        echo str_repeat('=', strlen('Firewall Rules API Coverage') + 8)."\n\n";
    }
}
