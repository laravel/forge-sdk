...
/**
 * Tests OpenAPI compliance for Providers endpoints.
 *
 * This test class dynamically validates that all Providers-tagged endpoints
 * in the OpenAPI spec have corresponding SDK methods and tests.
 */
class ProvidersTest extends OpenAPITestCase
{
    protected static string $category = 'Providers';

    public function test_all_providers_endpoints_have_sdk_methods()
    {
        ...
        if (! empty($uncovered)) {
            $this->fail(
                "The following ".count($uncovered)." Providers endpoints do not have SDK methods:\n".
                implode("\n", $uncovered)
            );
        }
    ...
    public function test_all_providers_sdk_methods_have_tests()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        ...
        if (! empty($missingTests)) {
            $this->fail(
                "The following ".count($missingTests)." Providers SDK methods do not have tests:\n".
                implode("\n", $missingTests)
            );
        }
    ...
    public function test_providers_endpoint_coverage_statistics()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        $testedMethods = static::getTestedMethods();
        ...
        echo str_repeat('=', strlen("Providers API Coverage") + 8) . "\n\n";
    }
}
