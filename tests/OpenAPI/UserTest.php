...
/**
 * Tests OpenAPI compliance for User endpoints.
 *
 * This test class dynamically validates that all User-tagged endpoints
 * in the OpenAPI spec have corresponding SDK methods and tests.
 */
class UserTest extends OpenAPITestCase
{
    protected static string $category = 'User';

    public function test_all_user_endpoints_have_sdk_methods()
    {
        ...
        if (! empty($uncovered)) {
            $this->fail(
                'The following '.count($uncovered)." User endpoints do not have SDK methods:\n".
                implode("\n", $uncovered)
            );
        }
    ...
    public function test_all_user_sdk_methods_have_tests()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        ...
        if (! empty($missingTests)) {
            $this->fail(
                'The following '.count($missingTests)." User SDK methods do not have tests:\n".
                implode("\n", $missingTests)
            );
        }
    ...
    public function test_user_endpoint_coverage_statistics()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        $testedMethods = static::getTestedMethods();
        ...
        echo str_repeat('=', strlen('User API Coverage') + 8)."\n\n";
    }
}
