...
/**
 * Tests OpenAPI compliance for Security Rules endpoints.
 *
 * This test class dynamically validates that all Security Rules-tagged endpoints
 * in the OpenAPI spec have corresponding SDK methods and tests.
 */
class SecurityRulesTest extends OpenAPITestCase
{
    protected static string $category = 'Security Rules';

    public function test_all_security_rules_endpoints_have_sdk_methods()
    {
        ...
        if (! empty($uncovered)) {
            $this->fail(
                "The following ".count($uncovered)." Security Rules endpoints do not have SDK methods:\n".
                implode("\n", $uncovered)
            );
        }
    ...
    public function test_all_security_rules_sdk_methods_have_tests()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        ...
        if (! empty($missingTests)) {
            $this->fail(
                "The following ".count($missingTests)." Security Rules SDK methods do not have tests:\n".
                implode("\n", $missingTests)
            );
        }
    ...
    public function test_security_rules_endpoint_coverage_statistics()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        $testedMethods = static::getTestedMethods();
        ...
        echo "\n\n";
        echo "=== Security Rules API Coverage ===\n";
        ...
        echo str_repeat('=', strlen("Security Rules API Coverage") + 8) . "\n\n";
    }
}
