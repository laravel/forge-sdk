...
/**
 * Tests OpenAPI compliance for Databases endpoints.
 *
 * This test class dynamically validates that all Databases-tagged endpoints
 * in the OpenAPI spec have corresponding SDK methods and tests.
 */
class DatabasesTest extends OpenAPITestCase
{
    protected static string $category = 'Databases';

    public function test_all_databases_endpoints_have_sdk_methods()
    {
        ...
        if (! empty($uncovered)) {
            $this->fail(
                'The following '.count($uncovered)." Databases endpoints do not have SDK methods:\n".
                implode("\n", $uncovered)
            );
        }
    ...
    public function test_all_databases_sdk_methods_have_tests()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        ...
        if (! empty($missingTests)) {
            $this->fail(
                'The following '.count($missingTests)." Databases SDK methods do not have tests:\n".
                implode("\n", $missingTests)
            );
        }
    ...
    public function test_databases_endpoint_coverage_statistics()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        $testedMethods = static::getTestedMethods();
        ...
        echo str_repeat('=', strlen('Databases API Coverage') + 8)."\n\n";
    }
}
