...
/**
 * Tests OpenAPI compliance for Sites endpoints.
 *
 * This test class dynamically validates that all Sites-tagged endpoints
 * in the OpenAPI spec have corresponding SDK methods and tests.
 */
class SitesTest extends OpenAPITestCase
{
    protected static string $category = 'Sites';

    public function test_all_sites_endpoints_have_sdk_methods()
    {
        ...
        if (! empty($uncovered)) {
            $this->fail(
                "The following ".count($uncovered)." Sites endpoints do not have SDK methods:\n".
                implode("\n", $uncovered)
            );
        }
    ...
    public function test_all_sites_sdk_methods_have_tests()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        ...
        if (! empty($missingTests)) {
            $this->fail(
                "The following ".count($missingTests)." Sites SDK methods do not have tests:\n".
                implode("\n", $missingTests)
            );
        }
    ...
    public function test_sites_endpoint_coverage_statistics()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        $testedMethods = static::getTestedMethods();
        ...
        echo str_repeat('=', strlen("Sites API Coverage") + 8) . "\n\n";
    }
}
