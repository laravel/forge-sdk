...
/**
 * Tests OpenAPI compliance for Logs endpoints.
 *
 * This test class dynamically validates that all Logs-tagged endpoints
 * in the OpenAPI spec have corresponding SDK methods and tests.
 */
class LogsTest extends OpenAPITestCase
{
    protected static string $category = 'Logs';

    public function test_all_logs_endpoints_have_sdk_methods()
    {
        ...
        if (! empty($uncovered)) {
            $this->fail(
                "The following ".count($uncovered)." Logs endpoints do not have SDK methods:\n".
                implode("\n", $uncovered)
            );
        }
    ...
    public function test_all_logs_sdk_methods_have_tests()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        ...
        if (! empty($missingTests)) {
            $this->fail(
                "The following ".count($missingTests)." Logs SDK methods do not have tests:\n".
                implode("\n", $missingTests)
            );
        }
    ...
    public function test_logs_endpoint_coverage_statistics()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        $testedMethods = static::getTestedMethods();
        ...
        echo str_repeat('=', strlen("Logs API Coverage") + 8) . "\n\n";
    }
}
