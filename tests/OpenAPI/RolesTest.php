...
/**
 * Tests OpenAPI compliance for Roles endpoints.
 *
 * This test class dynamically validates that all Roles-tagged endpoints
 * in the OpenAPI spec have corresponding SDK methods and tests.
 */
class RolesTest extends OpenAPITestCase
{
    protected static string $category = 'Roles';

    public function test_all_roles_endpoints_have_sdk_methods()
    {
        ...
        if (! empty($uncovered)) {
            $this->fail(
                "The following ".count($uncovered)." Roles endpoints do not have SDK methods:\n".
                implode("\n", $uncovered)
            );
        }
    ...
    public function test_all_roles_sdk_methods_have_tests()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        ...
        if (! empty($missingTests)) {
            $this->fail(
                "The following ".count($missingTests)." Roles SDK methods do not have tests:\n".
                implode("\n", $missingTests)
            );
        }
    ...
    public function test_roles_endpoint_coverage_statistics()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        $testedMethods = static::getTestedMethods();
        ...
        echo str_repeat('=', strlen("Roles API Coverage") + 8) . "\n\n";
    }
}
