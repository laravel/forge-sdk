...
/**
 * Tests OpenAPI compliance for Background Processes endpoints.
 *
 * This test class dynamically validates that all Background Processes-tagged endpoints
 * in the OpenAPI spec have corresponding SDK methods and tests.
 */
class BackgroundProcessesTest extends OpenAPITestCase
{
    protected static string $category = 'Background Processes';

    public function test_all_background_processes_endpoints_have_sdk_methods()
    {
        ...
        if (! empty($uncovered)) {
            $this->fail(
                'The following '.count($uncovered)." Background Processes endpoints do not have SDK methods:\n".
                implode("\n", $uncovered)
            );
        }
    ...
    public function test_all_background_processes_sdk_methods_have_tests()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        ...
        if (! empty($missingTests)) {
            $this->fail(
                'The following '.count($missingTests)." Background Processes SDK methods do not have tests:\n".
                implode("\n", $missingTests)
            );
        }
    ...
    public function test_background_processes_endpoint_coverage_statistics()
    {
        $endpoints = static::getEndpointsForTag(static::$category);
        $mapping = OpenAPIEndpointMapper::createMapping($endpoints);
        $testedMethods = static::getTestedMethods();
        ...
        echo "\n\n";
        echo "=== Background Processes API Coverage ===\n";
        ...
        echo str_repeat('=', strlen('Background Processes API Coverage') + 8)."\n\n";
    }
}
